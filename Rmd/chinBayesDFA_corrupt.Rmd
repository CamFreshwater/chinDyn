...
```{r groupFits}
## shuffle order of dum list to match re-arranged grouping
shuffSurv <- list(survList[4], survList[3], survList[1], survList[2])
names(shuffSurv) <- c("Subyearling\nSouth", "Subyearling\nSalish Sea",
                      "Yearling\nNorth", "Yearling\nSalish Sea" )

fitList <- lapply(seq_along(dum), function(x) {
    mod <- dum[[x]]$best_model
    p <- plot_fitted(mod) +
      ggtitle(names(shuffSurv)[x]) +
...
for(x in seq_along(fitList)) {
  fileName <- paste(abbreviate(names(shuffSurv)[x], 6), "3TrendFits.png", 
                    sep = "_")
